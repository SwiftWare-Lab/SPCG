...
int main(int argc, char** argv) {
    ...
    // Allocate and initialize data for L and U factors
    int *l_row_indices, *l_col_indices, *u_row_indices, *u_col_indices;
    float *l_values, *u_values;
    int l_num_rows, l_num_cols, l_num_nonzeros;
    int u_num_rows, u_num_cols, u_num_nonzeros;
    ...
    struct testOpts opts_l{};
    struct testOpts opts_u{};
    ...
    // For lower factor L
    const char *filename_l = argv[2];
    opts_l.sparse_mat_filename = (char *)malloc(strlen(filename_l) + 1);
    strcpy(opts_l.sparse_mat_filename, filename_l);
    printf("Using lower factor [%s]\n", opts_l.sparse_mat_filename);

    loadMMSparseMatrix<float>(opts_l.sparse_mat_filename, 'd', true, &l_num_rows,
                                   &l_num_cols, &l_num_nonzeros, &l_values, &l_row_indices,
                                   &l_col_indices, true);

    int base_l = 0;
    base_l = l_row_indices[0];  // base_l = {0,1}
    ...
    if (base_l) {
        ...
        for (int i = 0; i < l_num_nonzeros; i++) {
            l_col_indices[i]--;
        }
        base_l = 0;
    }

    std::cout << "Lower factor L is " << l_num_rows << " x " << l_num_cols << " with " << l_num_nonzeros << " nonzeros" << std::endl;

    // For upper factor U
    const char *filename_u = argv[3];
    opts_u.sparse_mat_filename = (char *)malloc(strlen(filename_u) + 1);
    strcpy(opts_u.sparse_mat_filename, filename_u);
    printf("Using upper factor [%s]\n", opts_u.sparse_mat_filename);

    loadMMSparseMatrix<float>(opts_u.sparse_mat_filename, 'd', true, &u_num_rows,
                                   &u_num_cols, &u_num_nonzeros, &u_values, &u_row_indices,
                                   &u_col_indices, true);

    int base_u = 0;
    base_u = u_row_indices[0];  // base_u = {0,1}
    if (u_num_rows != u_num_cols) {
        fprintf(stderr, "Error: only support square matrix\n");
        return 1;
    }
    ...
    if (base_u) {
        for (int i = 0; i <= u_num_rows; i++) {
            u_row_indices[i]--;
        }
        for (int i = 0; i < u_num_nonzeros; i++) {
            u_col_indices[i]--;
        }
        base_u = 0;
    }

    std::cout << "Upper factor U is " << u_num_rows << " x " << u_num_cols << " with " << u_num_nonzeros << " nonzeros" << std::endl;

    // Extract the parent directory from the L factor path
    std::string l_factor_path(filename_l);
    std::string parent_directory = l_factor_path.substr(0, l_factor_path.find_last_of("/"));
    ...
    std::string csv_filename = parent_directory + "/residuals_" + matrix_name + ".csv";

    // Open CSV file for writing
    std::ofstream outFile(csv_filename);

    std::cout << "Export csv has been set." << std::endl;
    ...
    int* d_col, * d_row;
    float* d_val, * d_x, * d_y, * d_r, * d_p, * d_omega, * d_zm1, * d_zm2, * d_rm2;
    ...
    int* d_l_row_indices, * d_l_col_indices, * d_u_row_indices, * d_u_col_indices, * d_pc_indices, * d_pr_indices;
    float* d_l_values, * d_u_values;

    // Allocate memory for L factor
    checkCudaErrors(cudaMalloc((void**)&d_l_row_indices, (l_num_cols + 1) * sizeof(int)));
    checkCudaErrors(cudaMalloc((void**)&d_l_col_indices, l_num_nonzeros * sizeof(int)));
    checkCudaErrors(cudaMalloc((void**)&d_l_values, l_num_nonzeros * sizeof(float)));

    // Allocate memory for U factor
    checkCudaErrors(cudaMalloc((void**)&d_u_row_indices, (u_num_cols + 1) * sizeof(int)));
    checkCudaErrors(cudaMalloc((void**)&d_u_col_indices, u_num_nonzeros * sizeof(int)));
    checkCudaErrors(cudaMalloc((void**)&d_u_values, u_num_nonzeros * sizeof(float)));

    // Copy matrices to device
    checkCudaErrors(cudaMemcpy(d_l_row_indices, l_row_indices, (l_num_cols + 1) * sizeof(int), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_l_col_indices, l_col_indices, l_num_nonzeros * sizeof(int), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_l_values, l_values, l_num_nonzeros * sizeof(float), cudaMemcpyHostToDevice));

    checkCudaErrors(cudaMemcpy(d_u_row_indices, u_row_indices, (u_num_cols + 1) * sizeof(int), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_u_col_indices, u_col_indices, u_num_nonzeros * sizeof(int), cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy(d_u_values, u_values, u_num_nonzeros * sizeof(float), cudaMemcpyHostToDevice));

    // Lower Part
    checkCudaErrors(cusparseCreateCsr(&matM_lower, l_num_rows, l_num_cols, l_num_nonzeros,
        d_l_row_indices, d_l_col_indices, d_l_values,
        CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
        CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F));
    ...
    // Upper Part
    checkCudaErrors(cusparseCreateCsr(&matM_upper, u_num_rows, u_num_cols, u_num_nonzeros,
        d_u_row_indices, d_u_col_indices, d_u_values,
        CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
        CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F));
    checkCudaErrors(cusparseSpMatSetAttribute(matM_upper, CUSPARSE_SPMAT_FILL_MODE, &fill_upper, sizeof(fill_upper)));
    checkCudaErrors(cusparseSpMatSetAttribute(matM_upper, CUSPARSE_SPMAT_DIAG_TYPE, &diag_non_unit, sizeof(diag_non_unit)));

    checkCudaErrors(cusparseSpSV_createDescr(&spsvDescrL));
    checkCudaErrors(cusparseSpSV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &floatone, matM_lower,
        vecR, vecY, CUDA_R_32F, CUSPARSE_SPSV_ALG_DEFAULT, spsvDescrL, &bufferSizeL));
    checkCudaErrors(cudaMalloc(&d_bufferL, bufferSizeL));

    checkCudaErrors(cusparseSpSV_createDescr(&spsvDescrU));
    checkCudaErrors(cusparseSpSV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &floatone, matM_upper,
        vecY, vecZM1, CUDA_R_32F, CUSPARSE_SPSV_ALG_DEFAULT, spsvDescrU, &bufferSizeU));
    ...
    cudaEvent_t start, stop, iterStart, iterStop, pureStart, pureStop;
    ...
    cudaEventCreate(&pureStart);
    cudaEventCreate(&pureStop);
    ...
    cudaEventRecord(pureStop);
    cudaEventSynchronize(pureStop);
    ...
    float milliseconds_pure = 0;
    cudaEventElapsedTime(&milliseconds_pure, pureStart, pureStop);
    printf("  Pure time for CG operation: %f ms\n", milliseconds_pure);
    ...
    if (outFile_sum.tellp() == 0) {
        outFile_sum << "Matrix Name,Parent Directory,Rows,Cols,Nonzeros,Final Residual,Iterations Spent,PCG Time (ms)" << std::endl;
    }

    // Append the results for this matrix to the CSV file
    outFile_sum << matrix_name << "," << parent_directory << "," << M << "," << N << "," << nz << ","
            << sqrt(r1) << "," << k << "," << milliseconds << std::endl;
    ...
    // Free device memory
    checkCudaErrors(cudaFree(d_l_row_indices));
    checkCudaErrors(cudaFree(d_l_col_indices));
    checkCudaErrors(cudaFree(d_l_values));
    checkCudaErrors(cudaFree(d_u_row_indices));
    checkCudaErrors(cudaFree(d_u_col_indices));
    checkCudaErrors(cudaFree(d_u_values));
